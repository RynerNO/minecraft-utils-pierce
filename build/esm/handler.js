...
import { copyFileSync, createWriteStream, existsSync, mkdirSync, readdirSync, readFileSync, unlinkSync, writeFileSync, } from 'node:fs';
...
class Handler {
    ...
    checkJava(java) {
        return new Promise((resolve) => {
            exec(`"${java}" -version`, (error, _, stderr) => {
                ...
                log('debug', `Using Java version ${stderr.match(/"(.*?)"/)?.pop()} ${stderr.includes('64-Bit') ? '64-bit' : '32-Bit'}`);
                resolve({ run: true });
            });
        });
    }
    ...
    async getVersion() {
        const versionJsonPath = this.config.versionJson || join(this.config.directory, `${this.config.version.number}.json`);
    ...
    async getJar() {
        ...
        writeFileSync(join(this.config.directory, `${this.config.version.number}.json`), JSON.stringify(this.parsedVersion, null, 4));
    ...
    async getAssets() {
        ...
        if (!existsSync(join(assetDirectory, 'indexes', `${assetId}.json`)))
            await this.downloadAsync(this.parsedVersion.assetIndex.url, join(assetDirectory, 'indexes'), `${assetId}.json`, true, 'asset-json');
        ...
        if (isLegacy(this.parsedVersion)) {
            if (existsSync(join(assetDirectory, 'legacy'))) {
                log('debug', "The 'legacy' directory is no longer used as Minecraft looks " +
    ...
    async getNatives() {
        const nativeDirectory = resolve(this.config.natives || join(this.config.root, 'natives', this.parsedVersion.id));
            ...
            const natives = async () => {
                ...
                await Promise.all(this.parsedVersion.libraries.map(async (lib) => {
                    ...
                    if (this.parseRule(lib))
                        return;
                    const native = getOS(this.config.os) === 'osx'
                        ? lib.downloads.classifiers['natives-osx']
                            ? lib.downloads.classifiers['natives-osx']
                            : lib.downloads.classifiers['natives-macos']
                        : lib.downloads.classifiers[`natives-${getOS(this.config.os)}`];
                    natives.push(native);
                }));
                return natives;
            };
    ...
    async getForgedWrapped() {
        ...
        if (json.mavenFiles)
            json.mavenFiles = cleanUp(json.mavenFiles);
    ...
    async getLaunchOptions(modification) {
        ...
        const assetPath = resolve(isLegacy(this.parsedVersion)
    ...
    getMemory() {
        if (typeof this.config.memory.min === 'number' && typeof this.config.memory.max === 'number') {
            ...
            log('debug', `MIN memory is a ${typeof this.config.memory.min} while MAX is ${typeof this.config.memory.max}! Resetting!`);
            this.config.memory = {
                min: Math.pow(2, 9),
                max: Math.pow(2, 10),
            };
