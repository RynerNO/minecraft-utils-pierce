...
export class Client {
    ...
    constructor(config) {
        ...
        this.config.directory = join(this.config.root, 'versions', this.config.version.custom || this.config.version.number);
    ...
    async install() {
        ...
        if (this.config.gameDirectory) {
            ...
            if (!existsSync(this.config.gameDirectory))
                mkdirSync(this.config.gameDirectory, { recursive: true });
        }
        ...
        if (this.config.version.custom) {
            ...
            modifyJson = JSON.parse(readFileSync(join(this.config.root, 'versions', this.config.version.custom, `${this.config.version.custom}.json`), {
                encoding: 'utf8',
            }));
        }
